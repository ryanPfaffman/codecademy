...
//I would've just compared to an uppercase char array for speed, but thought the UpperOrLower function was kinda cool to try.
...
namespace CaesarCipher
{
  class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      for (int x = 0; x < firstChange.Length; x++)
      {
        if (lowerOrUpper[x] == 1)
        {
          //do nothing
        ...
        {
          firstChange[x] = Convert.ToChar(firstChange[x].ToString().ToUpper());
        } else
        {
          //do nothing
        }
      ...
      string changeBack = String.Join("", ShiftBack(firstChangeS.ToCharArray()));
